...
const CartItem = ({product}: ICartItemProps) => {
    ...
    const stockQuantity = useAppSelector(state => selectStockQuantity(state.stockReducer.stock, product.id));
    ...
    const handleQuantityInput = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (Number(e.target.value) >=1 && stockQuantity && Number(e.target.value) <= stockQuantity) {
            dispatch(changeQuantity([Number(e.target.value), product.id]));
        }
    };
        ...
        >
            <div className="row">
                ...
                <div className="col-lg-6 d-flex align-items-center ps-5 my-3">
                    <h6 className="align-middle">{product.name}</h6>
                </div>
                <div className="col-lg-5 row text-center d-flex justify-content-center mx-auto">
                    ...
                    <div className="col-4 text-center d-flex align-items-center">
                        <h5>{product.properties.price * product.quantity} руб.</h5>
                    </div>
